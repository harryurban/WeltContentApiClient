...
class RawChannelReadsTest extends PlaySpec {
  ...
  "RawChannelReads" should {
    ...
    trait Fixture {
      ...
      val index = 1
      val originalTrackingName = Some("custom-stage")
    ...
    "read json" in new Fixture {
      ...
      stages must be(Seq(RawChannelStageCustomModule(index = index, module = moduleValue, overrides = overrides, trackingName = originalTrackingName)))
